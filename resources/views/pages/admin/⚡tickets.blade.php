...
<div class="space-y-6">
    ...
    <x-ui.modal name="ticket-create-modal" max-width="max-w-xl" :title="__('Create Ticket on Behalf of User')" :description="__('Log a support inquiry received via phone, in-person front desk, or direct email.')">
        <form wire:submit="createTicket" class="space-y-5 pt-1">
            ...
            <div class="space-y-1.5">
                <label class="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground block">
                    {{ __('Detailed Description') }} <span class="text-destructive">*</span>
                </label>
                <textarea
                    ...
                    placeholder="Provide full description, problem details, course reference, or contact notes..."
                ></textarea>
            ...
            <div class="flex items-center justify-end gap-2.5 pt-4 border-t border-border">
                <x-ui.button variant="outline" size="sm" type="button" x-data x-on:click="$store.modals.close('ticket-create-modal')">
                    {{ __('Cancel') }}
                </x-ui.button>
            ...
            @elseif ($bulkActionType === 'delete')
                <div class="p-3 rounded-lg bg-destructive/10 border border-destructive/20 text-destructive text-xs space-y-1">
                    ...
                    <p class="text-[11px] opacity-90">
                        {{ __('Are you sure you want to soft-delete :count selected tickets? They can be audited in deleted records.', ['count' => count($selectedIds)]) }}
                    </p>
                </div>
            @endif

            <div class="flex items-center justify-end gap-2.5 pt-4 border-t border-border">
                <x-ui.button variant="outline" size="sm" type="button" x-data x-on:click="$store.modals.close('ticket-bulk-modal')">
                    {{ __('Cancel') }}
                </x-ui.button>
                <x-ui.button :variant="$bulkActionType === 'delete' ? 'destructive' : 'default'" size="sm" type="submit">
                    {{ __('Apply Action') }}
                </x-ui.button>
            </div>
        </form>
